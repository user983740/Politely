...
public class PromptBuilder {
    ...
    private static final Map<SituationContext, String> CONTEXT_LABELS = Map.ofEntries(
            Map.entry(SituationContext.REQUEST, "요청"),
            Map.entry(SituationContext.SCHEDULE_DELAY, "일정 지연"),
            Map.entry(SituationContext.URGING, "독촉"),
            Map.entry(SituationContext.REJECTION, "거절"),
            Map.entry(SituationContext.APOLOGY, "사과"),
            Map.entry(SituationContext.COMPLAINT, "항의"),
            Map.entry(SituationContext.ANNOUNCEMENT, "공지"),
            Map.entry(SituationContext.FEEDBACK, "피드백"),
            Map.entry(SituationContext.BILLING, "비용/정산"),
            Map.entry(SituationContext.SUPPORT, "기술지원"),
            Map.entry(SituationContext.CONTRACT, "계약"),
            Map.entry(SituationContext.RECRUITING, "채용"),
            Map.entry(SituationContext.CIVIL_COMPLAINT, "민원"),
            Map.entry(SituationContext.GRATITUDE, "감사")
    );
    ...
    private static final Map<Persona, String> PERSONA_BLOCKS = Map.of(
            Persona.BOSS, """
                    ...
                    ## 받는 사람: 직장 상사
                    호칭: 인사 시 "안녕하세요" 또는 "안녕하십니까"로 시작. "상사님"/"팀장님"/"부장님" 등은 senderInfo에 직급이 있을 때만 사용. **"상장님" 등 잘못된 호칭 절대 금지**.
            ...
            Persona.PROFESSOR, """
                    ...
                    서명 "[이름] 올림". **캐주얼 표현 절대 금지**: "솔직히"→삭제 또는 "말씀드리자면", "빡빡하다"→"촉박하다", "꼬이다"→"이슈가 발생하다", "정리해놨고"→"정리하였으며"
    ...
    private static final Map<SituationContext, String> CONTEXT_BLOCKS = Map.ofEntries(
            Map.entry(SituationContext.REQUEST, """
                    - **요청**: 부담을 줄이는 완곡 표현("~해주시면 감사하겠습니다", "혹시 가능하시다면"). 상대의 바쁜 상황 배려("바쁘신 중에 죄송합니다만"). 기한은 원문의 구체적 날짜 유지. 요청 이유를 간결히 설명."""),
            Map.entry(SituationContext.SCHEDULE_DELAY, """
                    - **일정 지연**: 사과를 먼저("지연되어 죄송합니다") + 원인 간결히(변명 아닌 사실 설명) + 구체적 새 일정("~까지 완료하겠습니다"). 변명성 설명은 최소화하고 대안에 집중."""),
            Map.entry(SituationContext.URGING, """
                    - **독촉**: 이전 요청 사실을 상기시키되 비난하지 않기("앞서 말씀드린 건으로 확인 부탁드립니다"). 구체적 회신 기한 제시. "확인차 연락드립니다" 패턴으로 부드럽게. 반복 독촉이면 단호하되 예의 유지."""),
            Map.entry(SituationContext.REJECTION, """
                    - **거절**: 감사/이해 표현 → 거절 이유(솔직하되 부드럽게) → 가능한 대안 제시 → 아쉬움 마무리. "어렵겠습니다", "양해 부탁드립니다" 등 완곡 표현 활용."""),
            Map.entry(SituationContext.APOLOGY, """
                    - **사과**: 진심 어린 사과("깊이 사과드립니다") + 상대 불편 공감 + 원인(부적절한 사유는 "불가피한 사정"으로 대체) + 해결책/재발 방지 의지. 체념("어쩔 수 없다")은 개선 의지로 전환. 사과와 함께 구체적 보상/해결안 제시."""),
            Map.entry(SituationContext.COMPLAINT, """
                    - **항의**: 감정을 절제하고 사실 기반으로 문제 기술. 구체적 근거(날짜, 횟수, 금액) 제시. 원하는 해결 방향 명시("~해주시면 감사하겠습니다"). 공격적 표현 대신 건설적 요청으로."""),
            Map.entry(SituationContext.ANNOUNCEMENT, """
                    - **공지**: 핵심 정보(일시·장소·대상·내용)를 두괄식으로. 부가 설명은 뒤에 간결히. 행동 요청("참석 부탁드립니다", "확인 부탁드립니다")으로 마무리."""),
            Map.entry(SituationContext.FEEDBACK, """
                    - **피드백**: 긍정적 면을 구체적으로 먼저 언급 → 개선점을 건설적으로 제시("~하면 더 좋을 것 같습니다") → 함께 노력하는 자세. 비판이 아닌 성장 지향 톤."""),
            Map.entry(SituationContext.BILLING, """
                    - **비용/정산**: 금액·내역의 정확성 최우선. 단가·수량·합계 명시적으로 기재. 정산 기한과 입금 계좌 등 구체 정보 포함. 청구서/영수증 참조 안내."""),
            Map.entry(SituationContext.SUPPORT, """
                    - **기술지원**: 환경/버전 정보 명시. 재현 단계(1-2-3) 구조화. 로그/스크린샷 안내. 해결 진행상황 공유. 기술 용어는 정확히, 비기술 수신자에겐 쉽게 풀어서."""),
            Map.entry(SituationContext.CONTRACT, """
                    - **계약**: 조항 번호/명칭 정확히 참조. 법적 톤 유의("~에 의거하여", "~에 해당합니다"). 양측 의무/권리 명확히. 변경 시 서면 합의 필요성 언급."""),
            Map.entry(SituationContext.RECRUITING, """
                    - **채용**: 전문적이면서 환영하는 톤. 역할/직급/소속 명확히. 면접 일정/장소/준비물 구체적 기재. 문의 창구 안내."""),
            Map.entry(SituationContext.CIVIL_COMPLAINT, """
                    - **민원**: 법적 근거(관련 법률/조례) 참조. 처리 기한 명시. 담당자/부서 안내. 이의 신청 방법 고지. 격식체 유지."""),
            Map.entry(SituationContext.GRATITUDE, """
                    - **감사**: 구체적 행동/도움에 대한 감사 표현. 덕분에 얻은 효과/결과 언급. 향후 협력 의지. 따뜻한 마무리.""")
    );
