...
def test_emoji_detection():
    result = validate("안녕하세요 😊", "안녕하세요", None, None, Persona.BOSS)
...
def test_forbidden_phrase_detection():
    result = validate(
        "변환 결과입니다. 좋은 하루 되세요.", "원문", None, None, Persona.BOSS,
    )
...
def test_hallucinated_fact_number():
    result = validate(
        "총 50000원이 청구되었습니다.", "금액을 확인해주세요.", None, None, Persona.BOSS,
    )
...
def test_ending_repetition_detection():
    ...
    result = validate(text, "원문", None, None, Persona.BOSS)
...
def test_length_overexpansion():
    ...
    result = validate(output, original, None, None, Persona.BOSS)
...
def test_perspective_error_boss():
    result = validate(
        "확인해 드리겠습니다. 감사합니다.",
        "확인해 주세요", None, None, Persona.BOSS,
    )
...
def test_no_perspective_error_for_client():
    result = validate(
        "확인해 드리겠습니다.", "확인해 주세요", None, None, Persona.CLIENT,
    )
    warnings = [i for i in result.issues if i.type == ValidationIssueType.PERSPECTIVE_ERROR]
...
def test_locked_span_present_in_raw():
    ...
    result = validate(
        ...
        spans, "{{EMAIL_1}}으로 연락 부탁드립니다.", Persona.BOSS,
    )
    errors = [i for i in result.issues if i.type == ValidationIssueType.LOCKED_SPAN_MISSING]
    assert len(errors) == 0
...
def test_redaction_trace():
    result = validate(
        "내용입니다. [삭제됨] 확인 바랍니다.", "원문", None, None, Persona.BOSS,
    )
...
def test_core_number_missing():
    result = validate(
        "금액을 확인해주세요.", "50000원을 입금해주세요.", None, None, Persona.BOSS,
    )
...
def test_core_date_missing():
    result = validate(
        "기한을 확인해주세요.", "2024-03-15까지 제출해주세요.", None, None, Persona.BOSS,
    )
...
def test_validation_result_helpers():
    result = validate("안녕 😊 변환 결과", "안녕", None, None, Persona.BOSS)
